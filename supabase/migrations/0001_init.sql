...
create table if not exists public.measures (
  ...
  -- Which way is good. Inverts the achievement ratio for measures like debt
  -- ratios and lead times, where coming in under target is the win.
  direction      text not null default 'higher' check (direction in ('higher','lower')),
  -- Optional per-measure RAG thresholds, read against the raw reported value.
  -- When set they take precedence over banding the achievement percentage --
  -- see measureRating() in lib/calculations.js.
